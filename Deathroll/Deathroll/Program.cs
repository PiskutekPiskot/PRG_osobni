    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            while (a>1)
            {
                ...
                if (int.TryParse(zacatek, out int num)) // zjistí jestli je zacatek int, pokud ne tak int.TryParse(zacatek, out int num)= false
                {
                    a = Convert.ToInt32(zacatek);
                    int hodH = rnd.Next(1, a);
                    Console.WriteLine("hodil si " + hodH + "(1-" + a + ")");
                    a = hodH;
                    if (a==1)
                    {
                    ...
                    {
                        int hodP = rnd.Next(1, a);
                        Console.WriteLine("Počítač hodil " + hodP + "(1-" + a + ")");
                        a = hodP;
                        if (a == 1)
                        {
                            Console.WriteLine("Počítač prohál");
                        }
                    }
